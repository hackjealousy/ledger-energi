...
const bagl_element_t ui_verify_sign_mnb_nanos[] = {
    UI_NANOS_BACKGROUND(),
    UI_NANOS_ICON_LEFT(0, BAGL_GLYPH_ICON_CROSS),
    UI_NANOS_ICON_RIGHT(0, BAGL_GLYPH_ICON_CHECK),
    UI_NANOS_TEXT(1, 0, 12, 128, "Sign the", BAGL_FONT_OPEN_SANS_EXTRABOLD_11px),
    UI_NANOS_TEXT(1, 0, 26, 128, "broadcast?", BAGL_FONT_OPEN_SANS_EXTRABOLD_11px),

    UI_NANOS_TEXT(2, 0, 12, 128, "IP Address", BAGL_FONT_OPEN_SANS_REGULAR_11px),
    UI_NANOS_SCROLLING_TEXT(2, 23, 26, 82, vars.tmp.fullAddress, BAGL_FONT_OPEN_SANS_EXTRABOLD_11px)
};

unsigned int ui_verify_sign_mnb_nanos_button(unsigned int button_mask, unsigned int button_mask_counter);

unsigned int ui_verify_sign_mnb_prepro(const bagl_element_t *element) {
    if (element->component.userid > 0) {
        unsigned int display = (ux_step == element->component.userid - 1);
        if (display) {
            switch (element->component.userid) {
...
unsigned int io_seproxyhal_touch_sign_mnb_verify_cancel(const bagl_element_t *e) {
    // user denied the transaction, tell the USB side
    btchip_bagl_user_action_sign_mnb(0);
    // redraw ui
    ui_idle();
    return 0; // DO NOT REDRAW THE BUTTON
}
...
unsigned int
io_seproxyhal_touch_sign_mnb_verify_ok(const bagl_element_t *e) {
    // user accepted the transaction, tell the USB side
    btchip_bagl_user_action_sign_mnb(1);
    // redraw ui
    ui_idle();
    return 0; // DO NOT REDRAW THE BUTTON
}
...
unsigned int ui_verify_sign_mnb_nanos_button(unsigned int button_mask, unsigned int button_mask_counter) {
    ...
    switch (button_mask) {
    case BUTTON_EVT_RELEASED | BUTTON_LEFT:
        io_seproxyhal_touch_sign_mnb_verify_cancel(NULL);
        break;

    case BUTTON_EVT_RELEASED | BUTTON_RIGHT:
        io_seproxyhal_touch_sign_mnb_verify_ok(NULL);
        break;
    }
    return 0;
}
...
void btchip_bagl_confirm_sign_mnb() {
    // borrowing vars.tmp.fullAddress
    snprintf(vars.tmp.fullAddress, sizeof(vars.tmp.fullAddress), "%d.%d.%d.%d", btchip_context_D.mnc.mn_ip[0],
      btchip_context_D.mnc.mn_ip[1], btchip_context_D.mnc.mn_ip[2], btchip_context_D.mnc.mn_ip[3]);

    ux_step = 0;
    ux_step_count = 2;
    UX_DISPLAY(ui_verify_sign_mnb_nanos, ui_verify_sign_mnb_prepro);
}
